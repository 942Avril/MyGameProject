...
public class s_zstate
{
	...
	private int zs_n_bits;
	private int zs_maxbits;
	...
	public static s_zstate create(int bits)
	{
		...
		zs.zs_maxbits = (int)((bits!=0) ? bits : BITS);
		zs.zs_maxmaxcode = (int)(1L << zs.zs_maxbits);
		zs.zs_hsize = HSIZE;
		zs.zs_free_ent = 0;
...
nomatch:	if (this.output((int) this.s_write.zs_ent) == -1)
			...
			{
				this.zs_codetab[i] = (uint)(this.zs_free_ent++);
	...
	public int output(int ocode)
	{
		...
		if (ocode >= 0)
		{
			...
			this.zs_offset += (uint)this.zs_n_bits;
			if (this.zs_offset == (this.zs_n_bits << 3))
			{
				...
				if (this.s_write.writer.zwriter_fwrite(bp, (int)sizeof(byte), (int)bits) != (int)bits)
					return (-1);
		...
		else
		{
			if (this.zs_offset > 0)
			{
				...
				if (this.s_write.writer.zwriter_fwrite(this.zs_buf, (int)1, (int)this.zs_offset) != (int)this.zs_offset)
					return (-1);
...
middleing:
	...
	private int getcode()
	{
		...
		int leftb = (int)(this.s_read.reader.zlen - this.s_read.reader.zoff);
		int leftbits = (int)(leftb * 8);
		int tbits = (int)(leftbits - (this.s_read.zs_roffset & 7));
		...
		this.s_read.reader.zoff = (int)(this.s_read.zs_roffset / 8);
